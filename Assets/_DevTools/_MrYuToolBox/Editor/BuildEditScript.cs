...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void Check(int buildTargetIndex, BuildInfoData info)
    {
        ...
        if (Resources.FindObjectsOfTypeAll<WedSoulHue>()[0].SakeThe != info.BaseUrl)
        {
            isSame = false;
            Debug.Log("域名");
        }
        ...
        if (Resources.FindObjectsOfTypeAll<ADGrecian>()[0].MAX_SDK_KEY != info.Applovin_SDK_KEY)
        {
            isSame = false;
            Debug.Log("admanager key");
        }
        if (Resources.FindObjectsOfTypeAll<ADGrecian>()[0].MAX_REWARD_ID != info.Applovin_REWARD_ID)
        {
            isSame = false;
            Debug.Log("reward id");
        }
        if (Resources.FindObjectsOfTypeAll<ADGrecian>()[0].MAX_INTER_ID != info.Applovin_INTER_ID)
        {
            isSame = false;
            Debug.Log("inter id");
        }
        if (Resources.FindObjectsOfTypeAll<WedSoulHue>()[0].RoomTeam != info.GameCode)
        {
            isSame = false;
            Debug.Log("gamecode");
        }
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
        ...
        Resources.FindObjectsOfTypeAll<ADGrecian>()[0].MAX_SDK_KEY = info.Applovin_SDK_KEY;
        Resources.FindObjectsOfTypeAll<ADGrecian>()[0].MAX_REWARD_ID = info.Applovin_REWARD_ID;
        Resources.FindObjectsOfTypeAll<ADGrecian>()[0].MAX_INTER_ID = info.Applovin_INTER_ID;
        ...
        if (!GameObject.Find("MainManager") || !GameObject.Find("MainManager").GetComponent<SandalWineGrecian>())
        {
            if (GameObject.Find("MainManager"))
            {
                GameObject.Find("MainManager").AddComponent<SandalWineGrecian>();
            }
            else
            {
                GameObject mainObj = new GameObject("MainManager");
                mainObj.AddComponent<SandalWineGrecian>();
            }
        }
        GameObject.Find("MainManager").GetComponent<SandalWineGrecian>().BrightID = info.Adjust_APP_ID;

        if (!GameObject.Find("MainManager").GetComponent<OnlyUpGrecian>())
        {
            GameObject.Find("MainManager").AddComponent<OnlyUpGrecian>();
        }

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<OnlyUpGrecian>().appid = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<OnlyUpGrecian>().appid = info.PackageName;
#endif
        ...
        Resources.FindObjectsOfTypeAll<WedSoulHue>()[0].RoomTeam = info.GameCode;
        Resources.FindObjectsOfTypeAll<WedSoulHue>()[0].SakeThe = info.BaseUrl;
        Resources.FindObjectsOfTypeAll<WedSoulHue>()[0].SakeCrampThe = info.BaseUrl + CWinter.CrampThe;
        Resources.FindObjectsOfTypeAll<WedSoulHue>()[0].SakeWinterThe = info.BaseUrl + CWinter.WinterThe;
        Resources.FindObjectsOfTypeAll<WedSoulHue>()[0].SakePestThe = info.BaseUrl + CWinter.PestThe;
        Resources.FindObjectsOfTypeAll<WedSoulHue>()[0].SakeSandalThe = info.BaseUrl + CWinter.SandalThe;
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.BaseUrl + CWinter.WinterThe + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.BaseUrl + CWinter.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.BaseUrl + CWinter.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif
       ...
       (data) =>
       {
           ...
           //if (rootData.data.apple_pie != "apple")
           {
               ...
               if (GameObject.Find("WedLackGrecian"))
               {
                   DestroyImmediate(GameObject.Find("WedLackGrecian"));
               }
           }
       },
       () =>
