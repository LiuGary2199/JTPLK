...
public class LiraStud
{
    public static long Discern()
    {
        return AshTellurium(DateTime.Now);
    }

    public static long AshTellurium(DateTime datetime)
    {
    ...
    /// <returns></returns>
    public static string LiraPestSpinet(DateTime dt, string format)
    {
        return dt.ToString(format);
    }

    public static DateTime PatternGoLiraPest(long seconds)
    {
        DateTime startTime = TimeZone.CurrentTimeZone.ToLocalTime(new DateTime(1970, 1, 1)); // 当地时区
    ...
    /// <returns></returns>
    public static string PatternSpinet(long totalTime, string connector = ":")
    {
    ...
    /// <param name="totalTime"></param>
    /// <returns></returns>
    public static string PatternSpinet2(long totalTime)
    {
        int seconds = Mathf.Max((int)(totalTime % 60), 0);
        int minutes = Mathf.Max((int)(totalTime / 60) % 60, 0);
