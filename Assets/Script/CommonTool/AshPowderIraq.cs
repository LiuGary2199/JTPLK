...
{
    public static void Giant<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
...
public class AshPowderIraq : ObeySubstrate<AshPowderIraq>
{
    // Start is called before the first frame update
    public bool PeepCooper;
    public bool isGain;

    static public AshPowderIraq instance;
    ...
    /// <returns></returns>
    public Vector3 SapFocusScene(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject AshPresideRoomRender(Transform SelfTransform, List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float SceneGoDecor(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Decor180Go360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int AshDeathDecompose(Vector2 p1, Vector2 p2)
    {
        float angle = Decor180Go360(SceneGoDecor(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 SapUnknownSiltWish(Renderer Renderer)
    {
    ...
    public Vector2 SapUnknownSiltWish(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 SapDriverWish(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 SapGenreNarrowly()
    {

        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = SapWeightHabit();
        float screenH = SapWeightRevere();
        float x = -screenW / 2 + xa * screenW;
        float y = -screenH / 2 + ya * screenH;
        return new Vector2(x, y);
    }

    public int SapRotateSwing(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] AshBattleThorough(int total, int count)
    {
    ...
    public Dictionary<string, object> UnlikeGoMaturation(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string MaturationGoUnlike(Dictionary<string, object> Info)
    {
    ...
    public int BidLiraVent(string dateStart, DateTime dateEnd)
    {
    ...
    public int SecLiraVent(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan LiraVent(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 AshTwoAtDrug(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = AshTwoAtDrug(canvas, text, strFragmentIndex);
            Vector3 lastPos = AshTwoAtDrug(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = AshTwoAtDrug(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 AshTwoAtDrug(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float CarnivalDyPlus(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Like= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string TreetopDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static string SubsistDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Like;
