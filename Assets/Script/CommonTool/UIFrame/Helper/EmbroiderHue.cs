...
public class EmbroiderHue : MonoBehaviour
{
    /* 字段 */
    private static EmbroiderHue _Forecast;              //本脚本私有单例实例
    ...
    /// </summary>
    /// <returns></returns>
    public static EmbroiderHue AshForecast()
    {
        if (_Forecast == null)
        {
            _Forecast = new GameObject("_ResourceMgr").AddComponent<EmbroiderHue>();
        }
        return _Forecast;
    }
    ...
    /// <param name="isCatch"></param>
    /// <returns></returns>
    public GameObject FourDozen(string path, bool isCatch)
    {
